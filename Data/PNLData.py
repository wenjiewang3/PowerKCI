...
def PNLData(CI, dim_z, nums, seed, dim_x = 1, dim_y = 1, noise_scale=0.5, T_scale=0.2):
    np.random.seed(seed)
    [seed1, seed2] = np.random.randint(0, 100000, 2)
    Z = np.random.randn(nums, dim_z)
    if CI:
        X = PNL_process(Z, dim_x, nums, seed1, noise_scale)
        Y = PNL_process(Z, dim_y, nums, seed2, noise_scale)
    else:
        T = np.random.randn(nums, 1)
        X = PNL_process(Z, dim_x, nums, seed1, noise_scale)
        Y = PNL_process(Z, dim_y, nums, seed2, noise_scale) # , T, T_scale=T_scale
        # T = np.random.randn(nums, 1)
...
def PNL_process(Z, dim_X, nums, seed, noise_scale):
    dim_Z = Z.shape[-1]
    np.random.seed(seed)
...
if __name__ == '__main__':
    X, Y, Z = PNLData([1, 1, 7], nums=600, seed=12341, CI=False)
